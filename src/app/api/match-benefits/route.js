...
// AI-powered benefit matching function
...
// Fallback rule-based matching (unchanged from previous version)
function fallbackMatching(transcript, userProfile) {
  ...
  benefitSchemes.forEach(scheme => {
    ...
    // Location matching
    if (userProfile?.address?.state && scheme.level === 'National') {
      score += 0.2;
      matchReasons.push('National scheme applicable');
    }

    // Basic demographic matching
    if (keywords.some(k => ['farmer', 'किसान'].includes(k)) && scheme.category === 'Agriculture') {
      score += 0.4;
      matchReasons.push('Occupation match');
    }

    if (keywords.some(k => ['house', 'घर', 'home'].includes(k)) && scheme.category === 'Housing') {
      score += 0.4;
      matchReasons.push('Housing need identified');
    }

    if (keywords.some(k => ['health', 'medical', 'स्वास्थ्य'].includes(k)) && scheme.category === 'Healthcare') {
      score += 0.4;
    ...
    if (score > 0.3) {
      matches.push({
        ...
        matchReasons,
        eligibilityStatus: score > 0.7 ? 'likely_eligible' : 'needs_verification'
      });
    }
  });
  ...
  return {
    ...
    extractedProfile: {
      ...
      needs: keywords.filter(k => ['house', 'health', 'education', 'job'].includes(k))
    },
    matchedBenefits: matches.slice(0, 5),
    recommendations: 'Matches found using keyword analysis. For better results, please provide more details.',
    confidenceScore: 0.6,
    processingTime: Date.now(),
    fallbackUsed: true
  };
...
export async function POST(request) {
  try {
    const { transcript, userProfile, language, sessionId } = await request.json();

    if (!transcript || transcript.trim().length < 10) {
      return NextResponse.json({
        success: false,
        error: 'Transcript too short. Please provide more details about your situation.',
        errorCode: 'INSUFFICIENT_INPUT'
      }, { status: 400 });
    }

    console.log(`Processing benefit matching for session: ${sessionId}`);
    console.log(`User location: ${userProfile?.address?.city}, ${userProfile?.address?.state}`);
    ...
    console.log(`Found ${result.matchedBenefits?.length || 0} matching benefits`);
